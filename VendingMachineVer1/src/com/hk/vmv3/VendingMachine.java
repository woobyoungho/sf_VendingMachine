...
public class VendingMachine {
	/*
	 * private Drink[] drinks = new Drink[10]; // 명시적 선언부 // 이배열먼저 실행된다. 그리고 생성자를 통해
	 * 배열지정됨.
	 * 
	 * public VendingMachine() { //생성자를 통한 배열지정 drinks = new Drink[5]; }
	 */
	...
	public void sales(int idx, Customer cust) {
		// 해당 제품의 주소값 빼온다음
		// money값과 제품의 price값 비교,m
		// 금액이 부족하면 "금액이 부족합니다."
		// else 잔돈을 나타낸다.
		// idx 값이 마이너스 값 x

		/*
		 * System.out.println((answer+1) + "번 선택"); Drink dr = drinks.get(idx); if
		 * (money < dr.getPrice()) { System.out.println("금액이 부족합니다."); } else {
	...
	public void showDrinks() {
		/*
		 * 내가 한 포문 for(int i = 0; i < drinks.size(); i++) { System.out.println( (i+1) +
		 * ". " + drinks.get()); }
		 */

		/*
		 * int y = 1; for(Drink d : drinks) { System.out.printf("%d. %s\n", y++, d); }
		 * System.out.println("----------------");
		 */
